...
def netPrintXDC(netlist, designator):
    usedDesignators = []
    for node in netlist:
        listofpins = node[1]
        extraNets = 0
        ...
        for pins in listofpins:
            if( pins[1] == designator ):
                if( usedDesignators.count( node[0] ) == 0 ):
                    if( re.match('.*?([0-9]+)$', node[0]) ): # add brackets around bus names, this looks to see if the string ends in digits
                       portname = re.sub('\d+$', '', node[0])  # this deletes trailing digits...
                       portname = portname + '[' + re.match('.*?([0-9]+)$', node[0]).group(1) + ']' # inserts the digits in brackets...
                    else:
                       portname = node[0]
                    printLine = 'set_property PACKAGE_PIN ' + pins[0] + ' [get_ports {' + portname + '}]'
                    # 'NET \"' + node[0] + '\" LOC = ' + pins[0] + ';'
                    # now match against regex list
                    for tup in iostandardMaps:
                        if( re.search( tup[0], node[0] ) ):
                            printStd = 'set_property IOSTANDARD ' + tup[1] + ' [get_ports {' + portname + '}]'
                            # 'NET \"' + node[0] + '\" IOSTANDARD = ' + tup[1] + ';'
...
netPrintXDC(pinlist, designator)
